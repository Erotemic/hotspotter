...
def print_chiptable(hs_tables):
    ...
    cx2_gname = [gx2_gname[gx] for gx in cx2_gx]
    cx2_name  = [ nx2_name[nx] for nx in cx2_nx]
    cx2_stroi = [str(roi).replace(',','') for roi in cx2_roi]
...
def test_load_csv():
    db_dir    = GZ_ALL
    hs_tables = load_csv_tables(db_dir)

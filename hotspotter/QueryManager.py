...
class QueryManager(AbstractManager):
    ...
    def  assign_feature_matches_1vM(qm, rr, K, method):
        ...
        if isTFIDF: # TF-IDF voting is a little different
            ...
            qfx2_kdists_vote = qfx2_Kdists[:,0:K]
            qfx2_kdists_norm = np.tile\
                    (qfx2_Kdists[:,-1].reshape(num_qf, 1), (1, K) )
            qfx2_kweight = np.array\
                    ([score_function(vd,nd)\
                      for (vd, nd) in \
                      zip(qfx2_kdists_vote.flat, qfx2_kdists_norm.flat)\
                     ], dtype=np.float32)
        ...
        qfx2_Kaxs    = [ np.array(axs)[sqbit].tolist() \
                        for (axs, sqbit) in\
                        zip( qfx2_Kaxs_.flat, qfx2_Ksqbit_) ]
        # Clean Vote for Info
...
class QueryResult(AbstractManager):
    ...
    def cx_sort(res): # Sorted Chips
        ...
        return np.array([cx for cx in cx_sort_ if cx not in invalids],dtype=np.uint32)

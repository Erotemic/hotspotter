...
def test_data():
    ...
    #from __init__ import *
    #rsout = util.RedirectStdout()
    #rsout.start()
    #with util.RedirectStdout('test_data>'):
    hs = ld2.HotSpotter(db_dir)
    #record = rsout.stop()
    #print util.indent(record, 'test_data>')
    qcx = 0
...
if __name__ == '__main__':
    ...
    torepl = ["<type '", "'>", "<class '"]
    def val2typestr(val): return util.list_replace(str(type(val)), torepl)
    ...
    dict_ = test_locals
    exec(util.execstr_dict(dict_, 'test_locals'))
    ...
    run_expt_str = util.execstr_func(func)
    print('Try these: ')

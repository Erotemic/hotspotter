...
(print, print_, print_on, print_off,
 rrr, profile, printDBG) = __common__.init(__name__, '[par]', DEBUG=False)
# Python
...
def _calculate(func, args):
    printDBG('[par] * %s calculating...' % (multiprocessing.current_process().name,))
    result = func(*args)
    #arg_names = func.func_code.co_varnames[:func.func_code.co_argcount]
    #arg_list  = [n+'='+str(v) for n,v in izip(arg_names, args)]
    #arg_str = '\n    *** '+str('\n    *** '.join(arg_list))
    printDBG('[par]  * %s finished:\n    ** %s' %
...
def _worker(input, output):
    printDBG('[par] START WORKER input=%r output=%r' % (input, output))
    for func, args in iter(input.get, 'STOP'):
        printDBG('[par] worker will calculate %r' % (func))
        result = _calculate(func, args)
        printDBG('[par] worker has calculated %r' % (func))
        output.put(result)
        #printDBG('[par] worker put result in queue.')
    #printDBG('[par] worker is done input=%r output=%r' % (input, output))
...
def parallel_compute(func=None, arg_list=[], num_procs=None, lazy=True, args=None,
    ...
    if nTasks == 0:
        print('[par] ... No %s tasks left to compute!' % func.func_name)
        return None
    ...
    try:
        ...
        sys.stdout.flush()
        print('[par!] PARALLEL ERROR: %r' % ex)
        print('[par!] task_list: ')
        for task in task_list:
            print('  %r' % (task,))
            break
        print('[par!] common_args = %r' % common_args)
        print('[par!] num_procs = %r ' % (num_procs,))
        print('[par!] task_lbl = %r ' % (task_lbl,))
        sys.stdout.flush()
        raise
    return ret
...
def make_task_list(func, arg_list, lazy=True, common_args=[], output_dir=None):
    ...
    print('[par] Already computed %d %s tasks' % (nSkip, func.func_name))
    return task_list
...
def _compute_in_parallel(task_list, num_procs, task_lbl='', verbose=True):
    ...
    if verbose:
        ...
        for i in xrange(nTasks):
            done_queue.get()
        print('[par]  ... done')
    printDBG('[par] stopping children')
